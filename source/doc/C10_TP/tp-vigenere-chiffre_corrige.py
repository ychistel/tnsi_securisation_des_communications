...
##
## TP 2 du chapitre 10 (Déchiffrement de la méthode de Vigenère
...
def extraire_lettres(nom_fichier):
    ...
    for c in contenu:
        if ord(c) in range(ord("A"), ord("Z") + 1): # CORRIGÉ
            lettres += c # CORRIGÉ
    return lettres
...
def trouver_lettre_la_plus_frequente(l):
    ...
    for lettre in l[1:]:
        occurrences[lettre] = occurrences.get(lettre, 0) + 1 # CORRIGÉ
        if occurrences[lettre] > occurrence_max:
            occurrence_max = occurrences[lettre] # CORRIGÉ
            lettre_la_plus_frequente = lettre # CORRIGÉ
...
# trouvée précédemment afin de trouver le décalage.

def trouver_decalage(lettre_la_plus_frequente):
    lettre_E_en_clair = ord("E")
    lettre_E_chiffree = ord(lettre_la_plus_frequente) # CORRIGÉ
    return lettre_E_chiffree - lettre_E_en_clair # CORRIGÉ
...
def dechiffrement(nom_fichier, d1, d2, d3):
    ...
    for c in contenu:
        if ord(c) in range(ord("A"), ord("Z") +1): # CORRIGÉ
            ...
            lettre_en_clair = (lettre_chiffree - decalages[decalage_en_cours]) % 26 # CORRIGÉ
            # On retransforme le code obtenu en lettre.
            c = chr(lettre_en_clair + ord("A")) # CORRIGÉ
            # On passe au décalage suivant pour la lettre suivante.
            decalage_en_cours = (decalage_en_cours + 1) % 3 # CORRIGÉ
