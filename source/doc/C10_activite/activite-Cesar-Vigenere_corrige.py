...
def creer_dictionnaire_Cesar(d):
    dictionnaire_Cesar = {}
    for i in range(26):
        dictionnaire_Cesar[chr(i + ord("A"))] = chr((i + d)%26 + ord("A")) # CORRIGÉ
...
def chiffrement_Cesar(d, message_clair):
    message_chiffre = ""
    dictionnaire = creer_dictionnaire_Cesar(d) # CORRIGÉ
    for c in message_clair:
        message_chiffre = message_chiffre + dictionnaire.get(c,c) # CORRIGÉ
...
def dechiffrement_Cesar(d, message_chiffre):
    return(chiffrement_Cesar(-d, message_chiffre)) # CORRIGÉ
...
def creer_dictionnaires_chiffrement(clef):
    ...
    n = len(clef)
    for i in range(n):
        dictionnaires.append(creer_dictionnaire_Cesar(ord(clef[i])-ord("A"))) # CORRIGÉ
...
def applique_dictionnaire(dictionnaires, message_entree):
    ...
    for c in message_entree:
        ...
        if c in dictionnaires[i]:
            message_sortie = message_sortie + dictionnaires[i].get(c,c) # CORRIGÉ
...
print(chiffrement_Vigenere("GLCO", "JE SUIS VENU, J'AI VU, J'AI VAINCU. JULES CESAR"))
