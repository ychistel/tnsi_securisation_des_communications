...
alphabet = []
for i in range(26):
    alphabet.append(chr(ord("A")+i)) # CORRIGÉ
print(alphabet)
...
def creer_alphabet_permute():
    ...
    random.shuffle(alphabet_permute) # CORRIGÉ
...
for i in range(26):
	dictionnaire_chiffrement[alphabet[i]] = alphabet_permute[i] # CORRIGÉ
	dictionnaire_dechiffrement[alphabet_permute[i]] = alphabet[i] # CORRIGÉ
...
def chiffrement(message):
    ...
    for c in message:
        message_chiffre += dictionnaire_chiffrement.get(c,c) # CORRIGÉ
...
def dechiffrement(chiffre):
    message = ""
    for c in chiffre:
        message += dictionnaire_dechiffrement.get(c,c) # CORRIGÉ
    return(message)

print(chiffrement("BIENVENUE, C'EST L'ETE"))
print(dechiffrement(chiffrement("BIENVENUE, C'EST L'ETE"))) # CORRIGÉ