...
def chiffrement_xor(message,clef):
    ...
    for i in range(n):
       c.append(ord(message[i]) ^ ord(clef[i])) # CORRIGE
    return(c)
...
def dechiffrement_xor(message_chiffre,clef):
    ...
    for i in range(n):
        m = m + chr(message_chiffre[i] ^ ord(clef[i])) # CORRIGE
    return(m)

print(chiffrement_xor("ABC", "041"))
# [113, 118, 114]

print(dechiffrement_xor([113,118,114], "041"))
# ABC
...
def chiffrement_xor_vigenere(message,clef):
    ...
    for i in range(n):
        c.append(ord(message[i]) ^ ord(clef[j])) # CORRIGE
...
def dechiffrement_xor_vigenere(message_chiffre,clef):
    ...
    for i in range(n):
        m = m + chr(message_chiffre[i] ^ ord(clef[j])) # CORRIGE
        # On utilise le caractère suivant de la clef.
        j = (j+1) % k
    return(m)
...
print(chiffrement_xor_vigenere("Quelle est la clef du mystère ?",
"abracadabra") == [48, 23, 23, 13, 15, 4, 68, 4, 17, 6, 65, 13, 3, 82, 2,
15, 4, 2, 65, 6, 7, 65, 12, 27, 1, 21, 139, 19, 1, 65, 93])
# True

print(dechiffrement_xor_vigenere([48, 23, 23, 13, 15, 4, 68, 4, 17, 6, 65, 13,
3, 82, 2, 15, 4, 2, 65, 6, 7, 65, 12, 27, 1, 21, 139, 19, 1, 65, 93],
"abracadabra") == "Quelle est la clef du mystère ?")
# True
